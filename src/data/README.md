...
- `direct`: 사용자가 직접 사용한 도구, 언어, 프레임워크에만 붙입니다.
- `ai-assisted`: AI가 제안했거나 표현을 정리한 역량 라벨에 붙입니다.
- 세부 라이브러리 수준이고 포트폴리오에서 직접 강조할 필요가 낮은 항목은 Stack에서 제외합니다.
- Google Form 또는 이메일이 없으면 Contact 섹션은 렌더링하지 않습니다.
