...
export const fallbackContent: PortfolioContent = {
  ...
  workCases: [
    ...
    },
    {
      ...
      summary: '???? schema ??, table ??, batch upsert/update ??? ??? ??? DB ?? ??? ??.',
      details: ['schema-scoped execution ?? ??.', '?? DB ??? helper ??? ??.'],
    ...
    },
    {
      ...
      role: 'Data Workflow',
      summary: 'ICT ?? CSV ???? ?? ???? ??? ? ??? ???, ???, ?? ??? ?? ??.',
      details: ['USL/LSL/Spec ??? ?? ?? ??.', '???? ??? ??? ? ?? ??? ??? ??.'],
      keywords: ['Python', 'Pandas', 'Normalization'],
    },
    {
      company: 'Mining5000',
      title: '?? INT Dashboard / Data Handling',
      period: '2025',
